...
const TemplateCard = ({ item }: { item: any }) => {
  return (
    ...
    >
      ...
      {item.type === "strip" && (
        <div
          className={`${item.bgColor} w-32 md:w-44 p-2 md:p-3 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col gap-2 border border-[#2C2A29]/10 relative z-10`}
        >
          {item.images.map((img: string, i: number) => (
            <div
              key={i}
              className="aspect-4/3 bg-[#EFEBDE]/80 w-full overflow-hidden flex items-center justify-center relative"
            >
              <Image
                src={img}
          ...
          <div className="pt-2 flex justify-center items-center">
            <span
              className={`text-[7px] tracking-[0.4em] font-heading font-medium ${item.textColor}`}
            >
              HIKARA
            </span>
          </div>
      ...
      {item.type === "postcard" && (
        <div
          className={`${item.bgColor} w-64 md:w-96 p-2 md:p-4 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col border border-[#2C2A29]/10 relative z-10`}
        >
          ...
          <div className="pt-4 pb-1 flex justify-between items-center px-1">
            <span className="text-[7px] tracking-widest text-[#5A5550] uppercase">
              Premium Print
            </span>
            <span
              className={`text-[10px] tracking-[0.3em] font-heading ${item.textColor}`}
            >
              HIKARA
            </span>
          </div>
      ...
      {item.type === "grid" && (
        <div
          className={`${item.bgColor} w-64 md:w-96 p-2 md:p-4 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] grid grid-cols-2 gap-1 border border-[#2C2A29]/10 relative z-10`}
        >
          {item.images.map((img: string, i: number) => (
            <div
              key={i}
              className="aspect-square bg-[#EFEBDE]/80 overflow-hidden relative"
            >
              <Image
                src={img}
          ...
          <div className="col-span-2 pt-3 flex justify-center items-center">
            <span
              className={`text-[9px] tracking-[0.4em] font-heading ${item.textColor}`}
            >
              HIKARA
            </span>
          </div>
      ...
      {item.type === "mini" && (
        <div
          className={`${item.bgColor} w-44 md:w-60 p-3 md:p-5 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] flex flex-col border border-[#2C2A29]/5 relative z-10`}
        >
          ...
          <div
            className={`pt-6 pb-2 text-center font-heading text-[11px] tracking-widest ${item.textColor}`}
          >
            HIKARA STUDIO
          </div>
        </div>
      ...
      <div className="text-center mt-2 shrink-0">
        <h4 className="font-heading text-xl text-[#2C2A29] uppercase tracking-widest mb-1">
          {item.name}
        </h4>
...
export const ThemesSection = ({ items }: Props) => {
  ...
  return (
    <>
      <section
        id="themes"
        className="bg-[#F6F4F0] relative pt-24 md:pt-32 pb-12 md:pb-16 overflow-hidden"
      >
          ...
          <div className="w-full relative py-8 md:py-12 flex items-center perspective-[2000px]">
            ...
            <SmoothMarquee baseVelocity={1}>
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="flex shrink-0 items-center justify-around pr-8"
                >
      ...
      <section
        id="filters"
        className="max-w-6xl mx-auto px-6 pt-16 md:pt-20 pb-32 md:pb-48 relative z-20 bg-[#F6F4F0]"
      >
        <FadeUp>
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 md:mb-32 gap-6">
            ...
            <p className="text-xs md:text-sm text-[#5A5550] max-w-sm uppercase tracking-widest leading-loose">
              Setiap jepretan adalah kanvas. Pilih filter eksklusif yang
              menyempurnakan mood momen Anda.
            </p>
          </div>
        </FadeUp>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-x-24 md:gap-y-32">
          {themes.map((theme, idx) => (
            ...
            >
              ...
              <FadeUp
                delay={0.3}
                className="w-full md:w-1/2 pt-2 md:pt-12 flex flex-col justify-center"
              >
                <span className="text-[9px] tracking-[0.3em] font-sans text-[#8B5E56] mb-4 uppercase inline-block border border-[#8B5E56]/30 px-3 py-1 rounded-full w-fit">
                  Filter {idx + 1}
                </span>
                <h3 className="font-heading text-2xl md:text-3xl text-[#2C2A29] mb-4 group-hover:text-[#8B5E56] transition-colors duration-500">
                  {theme.name}
                </h3>
                <p className="text-sm text-[#5A5550] leading-loose tracking-wide">
                  {theme.desc}
                </p>
              </FadeUp>
            </div>
