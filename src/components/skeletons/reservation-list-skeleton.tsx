    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-white p-6 border border-[#2C2A29]/10 flex flex-col justify-between h-[120px]">
            <Skeleton className="h-3 w-24 rounded-none bg-[#2C2A29]/10" />
            <Skeleton className="h-8 w-16 rounded-none bg-[#2C2A29]/10 mt-4" />
          </div>
        ))}
      </div>
