...
import {
  UserAvatar,
  UserEmailDisplay,
} from "@/components/features/dashboard/user-info";
      ...
      <aside className="hidden md:flex w-64 bg-white border-r border-[#2C2A29]/5 z-30 flex-col pt-8 shrink-0 h-full sticky top-0 overflow-y-auto">
        <div className="px-8 mb-12">
          <Image
            ...
            height={36}
            alt="HIKARA"
            className="h-8 w-auto mix-blend-multiply opacity-90"
          />
          <p className="text-[9px] tracking-[0.3em] text-[#8B5E56] uppercase mt-2 font-medium">
            Panel Admin
          </p>
        </div>
      ...
      <main className="flex-1 relative z-10 flex flex-col h-full overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-[#2C2A29]/5 flex items-center justify-between px-6 sticky top-0 z-20 shrink-0">
          <div className="flex items-center gap-4">
            <MobileNav />
            <h2 className="text-[10px] tracking-[0.4em] uppercase font-medium text-[#8B5E56] hidden sm:block">
              Sistem Manajemen Hikara
            </h2>
            <Image
              src="/logo.png"
              width={80}
              height={24}
              alt="HIKARA"
              className="h-6 w-auto mix-blend-multiply opacity-90 md:hidden"
            />
          </div>
          ...
          <div className="flex items-center gap-4">
            <Suspense
              fallback={
                <Skeleton className="w-8 h-8 rounded-full opacity-20" />
              }
            >
              <UserAvatar />
            </Suspense>
